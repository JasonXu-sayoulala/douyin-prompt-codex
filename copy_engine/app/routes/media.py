...
def generate_media():
    ...
    if request.method == "GET":
        ...
        return render_template("media/generate.html", templates=templates, topics=topics)

    data = request.get_json(silent=True) if request.is_json else request.form

    topic_id = data.get("topic_id")
    template_id = data.get("template_id")
    media_type = data.get("media_type", "slideshow")
    ...
    try:
        ...
        contents = content_service.generate_contents(topic=topic, template=template, user_id=current_user.id)
        ...
        image_paths = _media().generate_images_from_copy(copy_text=content.body, num_images=num_images)
        ...
        if media_type == "slideshow":
            ...
            overlay_parts = [(content.title or "").strip(), (content.body or "").strip()]
...
def download_media(content_id):
    ...
    try:
        ...
        return send_file(video_path, as_attachment=True, download_name=f"douyin_video_{content_id}.mp4")
    except Exception as e:
